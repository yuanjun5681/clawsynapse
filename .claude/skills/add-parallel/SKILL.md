...
Update `container-agno/agent-runner/src/index.ts`:
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo nanoclaw-agent-agno:latest "Container OK"
```
...
4. Rebuild: `./container-agno/build.sh && npm run build`
