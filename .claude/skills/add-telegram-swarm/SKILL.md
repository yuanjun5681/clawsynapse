...
Read `container-agno/agent-runner/src/ipc-mcp-stdio.ts` and update the `send_message` tool to accept an optional `sender` parameter:
...
```bash
npm run build
./container-agno/build.sh  # Required — MCP tool changed
...
5. Remove `sender` param from MCP tool in `container-agno/agent-runner/src/ipc-mcp-stdio.ts`
...
8. Rebuild: `npm run build && ./container-agno/build.sh && launchctl unload ~/Library/LaunchAgents/com.nanoclaw.plist && launchctl load ~/Library/LaunchAgents/com.nanoclaw.plist`
